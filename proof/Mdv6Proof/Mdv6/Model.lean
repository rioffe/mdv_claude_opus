...
| `MathSymbols.swift` (82 ln) | `MathSymbols.registeredSymbols` (counts by kind), `preprocess` | **partial**: the fourteen C-07.2(b) *regex* rewrites are pinned as the ordered table in `Spec.commandRewrites` and their application is **not modelled** (regex semantics is not in Lean core) — carried by T-07 |
