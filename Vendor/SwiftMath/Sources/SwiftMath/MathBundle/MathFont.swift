...
private class BundleManager {
    ...
    private func registerCGFont(mathFont: MathFont) throws {
        guard let resourceBundleURL = MathFontsBundle.locate()?.path(forResource: mathFont.rawValue, ofType: "otf") else {
    ...
    private func registerMathTable(mathFont: MathFont) throws {
        guard let mathTablePlist = MathFontsBundle.locate()?.url(forResource: mathFont.rawValue, withExtension:"plist") else {
            throw FontError.fontPathNotFound
        }
...
// mdv6 patch 1 (see README.md): resolve `mathFonts.bundle` from the host app bundle first
// (`Contents/Resources/mathFonts.bundle`, C-13), then from the SwiftPM resource bundle, then from
// the vendored source tree by `#filePath` so `swift run`/`swift test` work unbundled.
enum MathFontsBundle {
    static let url: URL? = {
        if let u = Bundle.main.url(forResource: "mathFonts", withExtension: "bundle"), Bundle(url: u) != nil { return u }
        if let u = Bundle.main.resourceURL?.appendingPathComponent("mathFonts.bundle"), Bundle(url: u) != nil { return u }
        if let u = moduleBundleURL(), Bundle(url: u) != nil { return u }
        let source = URL(fileURLWithPath: #filePath)                  // …/Sources/SwiftMath/MathBundle/MathFont.swift
            .deletingLastPathComponent().deletingLastPathComponent()   // …/Sources/SwiftMath
            .appendingPathComponent("mathFonts.bundle")
        if Bundle(url: source) != nil { return source }
        return nil
    }()

    private static func moduleBundleURL() -> URL? {
        // `Bundle.module` traps when the SwiftPM resource bundle is absent; probe its known locations instead.
        ...
        candidates.append(Bundle(for: BundleManager.self).bundleURL.appendingPathComponent(name))
        candidates.append(Bundle(for: BundleManager.self).bundleURL.deletingLastPathComponent().appendingPathComponent(name))
        for c in candidates {
            if let b = Bundle(url: c), let u = b.url(forResource: "mathFonts", withExtension: "bundle") { return u }
        }
        return nil
    }

    static func locate() -> Bundle? { url.flatMap { Bundle(url: $0) } }
}
