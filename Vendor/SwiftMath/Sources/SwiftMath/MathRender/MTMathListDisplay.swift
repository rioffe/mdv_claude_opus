...
// MARK: - MTBoxDisplay (mdv6 patch 3)

/// Rendering of a list framed by `\boxed{…}`.
class MTBoxDisplay : MTDisplay {
    ...
    override func draw(_ context:CGContext) {
        ...
        context.saveGState();
        self.textColor?.setStroke()
        let rect = CGRect(x: self.position.x + self.lineThickness / 2,
                          y: self.position.y - self.descent + self.lineThickness / 2,
                          width: self.width - self.lineThickness,
                          height: self.ascent + self.descent - self.lineThickness)
    ...
    func updateInnerPosition() {
        self.inner?.position = CGPointMake(self.position.x + self.padding + self.lineThickness, self.position.y);
    }
}
