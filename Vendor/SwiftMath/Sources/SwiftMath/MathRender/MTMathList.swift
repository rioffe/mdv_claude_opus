...
public enum MTMathAtomType: Int, CustomStringConvertible, Comparable {
    ...
    case accent
    /// A boxed atom (`\boxed{…}`) - mdv6 patch 3 (see README.md)
    case boxed
...
public class MTMathAtom: NSObject {
    ...
    public init(type:MTMathAtomType, value:String) {   // mdv6 patch 2 (see README.md): public so the host can register symbols
        self.type = type
...
// MARK: - MTBoxed (mdv6 patch 3)
/** An atom with a frame drawn around the contained math list (`\boxed{…}`). */
public class MTBoxed: MTMathAtom {
    public var innerList:  MTMathList?
    
    override public var finalized: MTMathAtom {
        let newBoxed = MTBoxed(self)
