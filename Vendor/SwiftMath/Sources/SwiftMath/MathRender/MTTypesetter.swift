...
class MTTypesetter {
    ...
    func createDisplayAtoms(_ preprocessed:[MTMathAtom]) {
        ...
        for atom in preprocessed {
            switch atom.type {
                ...
                case .boxed:
                    // mdv6 patch 3: boxed is treated as Ord, like overline.
                    if currentLine.length > 0 {
                        self.addDisplayLine()
                    }
                    self.addInterElementSpace(prevNode, currentType:.ordinary)
                    ...
                    let boxed = atom as! MTBoxed?
                    let display = self.makeBoxed(boxed)
                    displayAtoms.append(display!)
                    currentPosition.x += display!.width;
                    if atom.subScript != nil || atom.superScript != nil {
                        self.makeScripts(atom, display:display, index:UInt(atom.indexRange.location), delta:0)
    ...
    // MARK: - Boxed (mdv6 patch 3)
    
    /// `\boxed{…}`: a frame of fraction-rule thickness with 0.35 em padding around the inner list (C-07.2).
    func makeBoxed(_ boxed:MTBoxed?) -> MTDisplay? {
        let innerListDisplay = MTTypesetter.createLineForMathList(boxed!.innerList, font:font, style:style, cramped:cramped)
        let padding = 0.35 * styleFont.fontSize
        let thickness = styleFont.mathTable!.fractionRuleThickness
        let boxDisplay = MTBoxDisplay(withInner: innerListDisplay, position: currentPosition, range: boxed!.indexRange)
        boxDisplay.padding = padding
        boxDisplay.lineThickness = thickness
        boxDisplay.ascent = innerListDisplay!.ascent + padding + thickness
        boxDisplay.descent = innerListDisplay!.descent + padding + thickness
        boxDisplay.width = innerListDisplay!.width + 2 * (padding + thickness)
        return boxDisplay
    }
