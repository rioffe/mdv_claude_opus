...
| `VIEWING` | Blocks rendered lazily; watcher armed on the path (R-05); find/TOC/bookmarks operate on the cached split. In-flight renders (diagram layout, math) are cancelled when the document changes. | `LOADING` | open of another file → `LOADING`; file changed on disk **with a readable read** → `RELOADING`; a read that fails or returns a transient empty (E-21) → stays `VIEWING`; file deleted → stays `VIEWING` (E-21); swipe-delete of the displayed history row → `LOADING` of the new first history entry (selecting route; position persisted; the deleted entry's snapshots dropped, R-18), or `EMPTY` when no rows remain (R-20); window close → `CLOSED` |
| `RELOADING` | New content replaces `rawMarkdown` in place; scroll position kept; text selection not preserved. Transient empty/undecodable reads are ignored (R-05, E-21). | watcher event, coalesced 50 ms; a further watcher event while `RELOADING` is processed exactly as one during `VIEWING` — a readable read replaces the in-flight reload and completes into `VIEWING`, an unreadable one is ignored and the state is unchanged | → `VIEWING` |
...
The table above is **total**: every state-and-event pair the application can be in is named, and a pair it does not name is unreachable from that state (a launch while `VIEWING`, a watcher event while `EMPTY`, a load outcome while `RELOADING`). No row leaves a reachable pair unspecified. (F-140)
...
struct ParsedDocument {            // computed once per load (R-04); equality on `raw`
    ...
    let tocHeadings: [TOCHeading]  // level 1…3, from each block's first line (rule 7)
}
...
7. `tocHeadings` contains each block whose trimmed text starts with `# `, `## `, or `### ` and is not a fence, using its first line only; a block MAY span more than one line and only its first line carries the marker (rule 1 makes the one-line form the ordinary case). `text` and `slugText` are computed from the heading **body** — that first line with its leading `#` run and the single space after it removed — so the marker never reaches the TOC, a bookmark title, or a slug; `text` is the body with inline Markdown stripped (C-12 rules) and math converted per C-07.3, while `slugText` is the same without the math conversion. (F-142, F-143)
...
Query construction: split the input on whitespace; drop the characters `" ( ) : * ^` from each token; discard tokens that become empty; wrap every survivor as `"token"*`; join with spaces (FTS5 implicit AND). Dropping is **deletion, not escaping**: the six characters are removed wherever they occur, so `a"b` and `ab` produce the same term and a query cannot distinguish the two (F-147, accepted — the six characters are FTS5 syntax and every survivor is a quoted literal prefix). A query with no surviving tokens performs no search and yields no results. Results MUST use `ORDER BY rank ASC, path COLLATE NOCASE ASC, path ASC LIMIT 80`, with `snippet(articles_fts, 1, char(2), char(3), '…', 14)` — U+0002/U+0003 bracket matched terms and the UI renders them highlighted. The two path keys make equal-rank ordering deterministic, including the 80-row boundary.
...
**Rule 3's colour values.** The names above map to the SVG 1.1 colour keywords, case-insensitively:
...
`lightgray` `#d3d3d3` · `lightgrey` `#d3d3d3` · `darkgray` `#a9a9a9` · `silver` `#c0c0c0` ·
`pink` `#ffc0cb` · `lightblue` `#add8e6` · `lightgreen` `#90ee90` · `lightyellow` `#ffffe0` ·
`gold` `#ffd700` · `teal` `#008080` · `navy` `#000080` · `maroon` `#800000` · `olive` `#808000` ·
`cyan` `#00ffff` · `magenta` `#ff00ff` · `brown` `#a52a2a` · `beige` `#f5f5dc` · `ivory` `#fffff0` ·
`lavender` `#e6e6fa` · `coral` `#ff7f50` · `salmon` `#fa8072` · `tomato` `#ff6347` ·
`crimson` `#dc143c` · `indigo` `#4b0082` · `violet` `#ee82ee` · `khaki` `#f0e68c` · `tan` `#d2b48c` ·
`wheat` `#f5deb3` · `mintcream` `#f5fffa` · `honeydew` `#f0fff0` · `aliceblue` `#f0f8ff` ·
`whitesmoke` `#f5f5f5` · `gainsboro` `#dcdcdc` · `snow` `#fffafa`; `transparent` and `none` both map to
`#00000000`. The list is exhaustive: any other name is passed through (and renders black). (F-139)
...
Applied to one block; the block is returned unchanged if it is a fence, is a GFM table by **R-24's test** (first line contains `|`; the second line, trimmed, is non-empty, contains at least one `-`, and consists only of `-`, `:`, `|`, space), or is a thematic-break line. Otherwise, outside inline code spans (a run of $n$ backticks closes only on a run of exactly $n$), link/image URL parts (`](` … matching `)`), and `<…>` spans: `"` and `'` → directional quotes chosen from the preceding character — an **opening** quote when the preceding character is absent, whitespace, or one of `( [ { < “ ‘ — – - /`, and a **closing** quote otherwise (F-144; the predicate is normative, not illustrative); `---` → `—`; a run of **four or more** dashes is left unchanged; `--` between letters/digits → `–`; ` -- ` → ` — `; other `--` runs unchanged (CLI flags survive); `...` → `…` (a longer dot run keeps its extra dots).
...
`section(headingAt i)` = blocks $[i, j)$ where $j$ is the index of the next **TOC heading** (C-02 rule 7 — an h4–h6 or setext heading never ends a section) with level $\leq$ the level of $i$, or the block count. Copy output = those blocks joined with `\n\n`. `stripInlineMarkdown` removes trailing `#`s, `**`, `__`, backticks, unescaped `*`, **both** underscores of an `_…_` emphasis pair whose opening `_` is not preceded by a letter or digit (word-internal underscores such as `snake_case` are kept), and reduces `[text](url)` to `text`. The removals are applied in **this order**, which is normative because the order is observable (F-141): (1) trailing `#`s, and the surrounding whitespace is trimmed; (2) `[text](url)` → `text`; (3) the `_…_` pairs; (4) a left-to-right scan that drops `**` pairs, every backtick, every `_` of a `__` pair, and every unescaped `*`, keeping the escaped character alone when it follows a backslash (`*`, `_`, backtick, `#`, `[`, `]`, `\`); (5) the surrounding whitespace is trimmed again.
...
- *v0.13.2 (2026-09-25): F-148, reported against a downstream document rendered in mdv6 (`gpu-quicksort-revisited/SPEC.md` §7, K-06): `\lVert`/`\rVert` name amsmath's open/close variants of U+2016 — commands KaTeX, MathJax and GitHub all accept — but the vendored SwiftMath's delimiter and symbol tables know only `\Vert`/`\vert`, so every norm in that document took the E-10 fallback and showed its source. C-07.2 (b) gains `\lVert`/`\rVert` → `\Vert` and `\lvert`/`\rvert` → `\vert`, applied after `\not=` → `\neq` and before the size-command removal (statement 7 of the order), so the table is fourteen entries and `\left\lVert … \right\rVert` rewrites to a delimiter SwiftMath accepts. T-07's corpus carries the norm (`test-docs/math.md`), and the C-07.2 unit and render tests pin the four rewrites, the `\lVertneqq` boundary and the reported expression. No id renumbered.*
- *v0.13.1 (2026-09-24): the spec-model findings applied (F-139…F-147), from the Lean model of this specification (`proof_from_spec/`). Each finding was a place the document left a cell silent, a pin too loose, or a witness missing; the model's kernel-checked witnesses are in `docs/reviews/SPEC_MODEL_FINDINGS.md`. P1: F-139 — C-06.1 rule 3 named 46 CSS colour names and pinned no value, so `#eee`/`white` could render differently in two conforming builds (the SVG 1.1 keyword table is now normative); F-140 — §3.1's table was partial, leaving a reachable `EMPTY`+delete-row and `RELOADING`+event cell unstated (both now named, and the table is declared total); F-143 — C-12 removes trailing `#`s and nothing removed the leading ATX marker, so `TOCHeading.text` kept `# ` while T-08 expected the bare heading text (C-02 rule 7 now says the text and slugText come from the heading body); F-146 — C-07.1 pinned a span's acceptance but not the scan that finds spans, which no §9 test carried (the scan is now stated, so T-07 covers it). P2: F-141 — `stripInlineMarkdown`'s removals had no order and the order is observable; F-142 — "uses its first line only" (rule 7) contradicted the `tocHeadings` comment's "single-line ATX only"; F-144 — a quote's direction was "chosen from the preceding character" with no mapping; F-145 — C-10 defined a GFM table by example while R-24 defined it exactly; F-147 — "drop the characters from each token" deletes them, so `a"b` and `ab` collide (now documented as accepted). No id renumbered; no row removed.
- *v0.13 (2026-09-19): seven more highlighted languages (R-43, D-45). New R-43: `cpp` (+ `c++`/`cc`/`metal`), `json`, `lua`, `opencl` (+ `cl`), `perl` (+ `pl`) and `markdown` (+ `md`/`gfm`) fences highlight from six more upstream grammars (seven vendored directories) under `mdv6/Grammars/` — `tree-sitter-cpp` v0.23.4, `tree-sitter-json` v0.24.8, `tree-sitter-grammars/tree-sitter-lua` v0.5.0, `lefp/tree-sitter-opencl` (pinned by commit; the repository has no tag), `tree-sitter-perl` v2.0.0 (release tarball) and `tree-sitter-grammars/tree-sitter-markdown` v0.5.3 (block and inline) — pinned in its README together with the files that are derived rather than verbatim. C++ also backs a `metal` fence, because Metal Shading Language is C++14-based and the Metal grammars in the wild ship no licence file; markdown is the one language run as two grammars, the block grammar's captures followed by the inline grammar's over each `(inline)` span (C-05, D-45). K-05, C-05, §9.0, §10 and §11 carry the additions; T-51 proves them. No id renumbered.*
