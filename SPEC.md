...
struct ParsedDocument {            // computed once per load (R-04); equality on `raw`
    ...
    let blockLines: [Range<Int>]   // 1-based, inclusive-exclusive source lines per block (C-02 rule 8)
...
8. `blockLines[i]` is the range of **1-based** source line numbers that block `i` occupies, and `lineCount` is the number of lines the normalized input splits into (rule 6; a trailing `\n` does not create a final empty line). The map is derived from the same split as `blocks`, so `blocks[i]` is exactly `raw`'s lines `blockLines[i]`, with rule 5's trimming applied only inside the block text. Both are computed once per load and stored on the document, so a line→block lookup (C-19) is $O(\log |\mathrm{blocks}|)$ and answers at most once per distinct `raw` (I-004). A line number that no block covers — one past the last line, or inside the run of blank lines a rule-5 drop or a C-07.1 insertion removed — MUST resolve to the **last** block whose range starts at or before it.
...
**C-19.1 Grammar.** A line fragment is the fragment of a link destination, after the single UTF-8 percent-decode of R-19, matching

```
^L([0-9]+)(?:-(?:L)?([0-9]+))?$        case-insensitive, anchored
```

so `#L10`, `#L10-L12`, `#l10-l12`, and `#L10-12` all parse. Line numbers are **1-based** and refer to the target document's source lines after C-02 rule 6 normalisation — the same coordinate space as `blockLines`/`lineCount`. A fragment that does not match this pattern in full (including `#L`, `#L0`, `#L10C5`, `#Ll0`, and any fragment with a trailing or interior token) is **not** a line fragment and falls through to C-11 slug matching (R-19). `#L0` is not a citation: line numbers start at 1.
...
| **K-05** | Highlighted languages: C, Go, Rust, Bash, JavaScript, YAML, TOML, Python, Ruby (grammar commits pinned in `mdv6/Grammars/README.md`); Swift and SQL are required additions (R-38). |
...
| **E-29** | The TOC selected row (C-18.8) after scrolling away, after a reload (R-05), and after loading another document. | Scrolling does not move or clear it (selection is choice-driven, not viewport-driven — D-41). A reload keeps it when the heading still exists at that block index, else clears it. Loading a different document clears it; a cross-file `#fragment` (R-19) that lands on a TOC heading selects that row. |
...
| **E-31** | A line citation whose line does not exist in the target: past `lineCount`, inside a gap the C-02 split removed, or `#L0`. | `#L0` is not a citation (C-19.1) and falls through to E-06. A line past `lineCount` or in a gap resolves to the last block (C-02 rule 8) and is scrolled to and flashed like any other citation — no error, and never a refusal to load the target file. A line citation into a document with no blocks scrolls to the top and flashes nothing. |
...
| **Render snapshot** (`Tests/mdv6RenderTests`) | `MDVMermaidPipeline.prepare/rasterize`, `MathImageCache`, `CodeRenderer.render` against the C-17 corpus and manifest; PNG comparisons use C-17's dimensions and $q \leq 0.001$ rule | T-06, T-13, T-17 (ink measurement), T-18, T-19 | `swift test`, CI (macOS runner) |
...
| **T-49** | In `test-docs/links-sibling.md`, `[cite](links-sibling.md#L10-L12)` loads the sibling, scrolls the paragraph containing line 10 to the top of the viewport, and flashes that one block for 0.6 s in the R-22 accent tint; the flash restarts on a second click. Same-document `#L10` does the same without a load and **without** pushing a back snapshot, while `#second-heading` still pushes one. `#l10-l12`, `#L10-12`, and `#L12-L10` resolve identically to `#L10-L12` (case-insensitive; reversed range read as named). `#L10C5`, `#L`, `#L0`, and `#nope` are not citations: `#L0`, `#L10C5`, `#L` and `#nope` are all E-06 no-ops when the file has no such heading, and `#L0` never flashes. A citation past the sibling's `lineCount` and one naming a line inside the removed blank-line gap both scroll to and flash the **last** block (E-31) rather than failing. A citation into a document with no blocks scrolls to the top and flashes nothing. The TOC selected row is set by a slug fragment and **not** by a line citation (E-29, C-19.4). The separated `blockLines`/`lineCount` maps are asserted directly for a document whose blocks are a heading, a two-line paragraph, a blank-line-separated paragraph, an unclosed fence at EOF, and a CRLF variant. Proves R-19, C-02 rule 8, C-19, E-06, E-31. |

### 9.6 Visual parity (C-18, I-014, I-015)

A conformance report for this specification MUST include the outcome of T-44, T-47 and T-48 as observed on a screen by a person, or in a screenshot the person has looked at (T-48's menu order, enablement and reorder are also proved in `swift test`); a build whose §9.1–9.5 tests are green but whose T-44 was not observed is *verification pending*, not conforming (D-40). Where a host cannot show a window, the report says so and the status stays pending. Observed tests — and every §9 test that opens, deletes or reorders history, bookmarks or the placeholder (T-25, T-27, T-47, T-48) — run against an **isolated store** seeded from a fixture (`MDV6_SUPPORT_DIR`, `MDV6_DEFAULTS_SUITE`, §10), never against the reader's own.
...
| tree-sitter grammars (9) | commits in `mdv6/Grammars/README.md`, vendored C sources compiled as target `CGrammars` | code highlighting |
...
| C-02 | `ParsedDocument.parseBlocks/parseTOC`, `blockLines`/`lineCount` (rule 8) | T-07, T-30, T-39, T-49 |
...
| C-19 | `LineCitation.parse/resolve` (grammar, range normalisation, line→block), `DocumentSession.handleLink` (dispatch, scroll, flash) | T-49 |
...
| E-31 | `LineCitation.resolve` clamping to the last block; `#L0` fall-through in `DocumentSession.handleLink` | T-49 |
