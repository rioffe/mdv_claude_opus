...
> - **Gate:** `swift test --parallel --xunit-output junit.xml` → 173 tests, 0 failures; speccheck Phase A `CONFORMING` 175/175, exit 0. Phase B (LLM judge) is *not* clean — see §7 for the pre-existing weak rows and why none of them is this change.
...
| F-014 | `mdv6/Grammars/opencl/src/parser.c:1` (W9) | `#include <tree_sitter/parser.h>` — the angled form is what the old generator emitted, and it does not resolve inside `CGrammars` (which exposes only `include/`), so the target failed to compile. | Quoted include, recorded in `mdv6/Grammars/README.md` §Derivations. `swift build` clean. |
| F-015 | `mdv6/Queries/perl-highlights.scm` (W9) | Upstream `tree-sitter-perl` v2.0.0 uses the Neovim-only `#lua-match?` predicate on the shebang pattern. The tree-sitter 0.25 runtime does not know that predicate; the query must not depend on it. | `#match?`, one predicate, recorded in the README §Derivations. |
| F-016 | `mdv6/Queries/cpp-highlights.scm` (W9) | Upstream `tree-sitter-cpp`'s `highlights.scm` is 1 KB and captures keywords, strings and CamelCase types only — it is written to be used with an inherited C query (nvim-treesitter's `; inherits: c`). Vendored alone it would leave comments, numbers, operators and function names plain. | The `c` query followed by the `cpp` query, in one file (README §Derivations). Verified by capture dump before wiring: 10 distinct capture classes on a Metal sample. |
| F-017 | `mdv6/Queries/markdown-highlights.scm`, `markdown-inline-highlights.scm` (W9) | Upstream's markdown queries speak the Neovim `@text.*` vocabulary, for which `CodePalette` (C-09) has no key — every markdown capture would have resolved to `plain`, i.e. a `markdown` fence would look unhighlighted. | The six `@text.*` names are mapped onto palette keys in the derived queries (README §Derivations). Verified on screen (§5c). |
| F-018 | `Tests/mdv6Tests/BuildAndLauncherTests.swift` (W9) | The bundle-layout test pinned **11** `-highlights.scm` files in `Contents/Resources` — a K-05/C-13 contract that the wave legitimately changes. | Count is 18 (17 fence languages; markdown contributes two), and the three new query files are named explicitly. The test's doc comment cites K-05, R-38, R-43. |
| — | `mdv6/Queries/*.scm` (W9, observation) | Capture names upstream uses that no `CodePalette` key covers render in the plain colour (C-05). Pre-existing for the eleven: `c`'s `@delimiter`, `sql`'s `@conditional`/`@field`, `ruby`/`python`'s `@escape`, and now `lua`'s `@repeat`/`@conditional` and `perl`'s `@include`/`@conditional`. | Left as-is deliberately: the queries are upstream's, and the alternative — extending the palette — is a theme change (`TYPOGRAPHY.md`, four palettes) outside R-43. Recorded in R-43's wording and in D-45 rather than fixed silently. The markdown queries are the exception, because there the vocabulary gap would have removed the feature entirely (F-017). |
...
## 5c. The observed pass for W9 (seven more languages)

Run on the built bundle at `8e0b200`, isolated store (`MDV6_SUPPORT_DIR` / `MDV6_DEFAULTS_SUITE`), Sevilla theme, on the spec's own fixture `test-docs/code.md` — the file T-51 names.

| What was driven | How | Outcome |
| --- | --- | --- |
| The fixture at the top | `tools/observe.sh T-51-code-start test-docs/code.md --theme sevilla` | `build/observed/T-51-code-start.png` — the pre-existing languages, unchanged |
| `cpp`, `metal`, `opencl`, `json` | `tools/observe.sh drive action scrollTo 14` (then `17`) + `snap` | `build/observed/T-51-code-cpp.png`, `T-51-code-json.png` — read back by the agent: every block carries its label and its tokens are coloured (comments green, keywords red, identifiers blue, strings green, `42`/`2.0f`/`1.5f` blue, JSON `true` distinct); `kernel void`, `__global const float *` and `[[buffer(0)]]` all render |
| `lua`, `perl` | `scrollTo 18` + `snap` | `build/observed/T-51-code-lua-perl.png` — captured; the agent's read of it was interrupted |
| `markdown` (block + inline) | `scrollTo 20` + `snap` | `build/observed/T-51-code-markdown.png` — captured, then **looked at by the user in the running app**, who confirmed the result ("looks good") |

**Honest limits.** The `scrollTo` drive action sets `topVisibleBlock`, so a capture is the window's viewport rather than the whole file, and the `scrollTo 20` capture of the `markdown` block came out at the top of the document (the drive notification did not take on that launch); the markdown block's colours were therefore judged by the user in the running app rather than from that file. The lua/perl capture was taken but not read back by the agent. T-51 is a scripted row of §9.0's render group: its assertions (six capture classes per language, the alias equivalences, the markdown block-plus-inline captures, and a non-ASCII case that fails if the inline pass's offsets were byte-based rather than UTF-16) live in `swift test` and passed on this tree; the captures exist so the colours can be seen.
...
| R-43 | PASSING | CodeLanguage.swift, CodeRenderer.swift | `testAddedLanguagesCaptureClasses`, `testMarkdownBlockAndInlineCaptures`, `testAllGrammarsAndQueriesLoad` (CodeRendererTests); `testLanguageResolution` (MiscContractTests); `testBuiltBundleLayoutAndSignature` (BuildAndLauncherTests) |
...
| K-05 | PASSING | CodeLanguage.swift, CodeRenderer.swift | `testAllGrammarsAndQueriesLoad` (CodeRendererTests); `testNineLanguagesColourAndUnknownIsPlain` (CodeRendererTests); `testAddedLanguagesCaptureClasses` (CodeRendererTests); `testMarkdownBlockAndInlineCaptures` (CodeRendererTests); `testLanguageResolution` (MiscContractTests); `testBuiltBundleLayoutAndSignature` (BuildAndLauncherTests) |
...
| T-51 | PASSING | — | `testAddedLanguagesCaptureClasses`, `testMarkdownBlockAndInlineCaptures` (CodeRendererTests); `testLanguageResolution` (MiscContractTests); `testBuiltBundleLayoutAndSignature` (BuildAndLauncherTests); observed: `build/observed/T-51-code-*.png` (§5c) |
...
speccheck (llm):  PENDING — the Phase B run on this tree (deepseek/deepseek-v4.1-flash via OpenRouter) is recorded below when it lands; the run on the pre-markdown tree (169/175, 6 weak, unknown_rate 0.2316) named only pre-existing manual rows, none of them a W9 id
